...
```{r}
# Los numeros de 1 a 100, con intervalos de 1
uno.a.cien<-seq(1,100,1) 
uno.a.cien
```
...
```{r}
# 5 repeticiones del numero 10
reps1<-rep(10,5)
reps1
# 25 repeticiones del número 2
reps2<-rep(2,25)
reps2
```
...
```{r}
datos_u <- data.frame(meses = month.name[1:5],
                      numeros = 1:5,
                      logico = c(FALSE, TRUE, FALSE, FALSE, FALSE))

datos_u

# PARA CAMBIAR EL NOMBRE DE LAS COLUMNAS  
colnames(datos_u) <- c("MESES","NUMEROS", "LOGICO")
...
pacientes<-c("juan", "pedro", "armando", "eliel",
             "oscar", "roberto","carlos")
pacientes
...
```{r}
pacientes2<-c("Juana", "Pedro", "Armando",
              "Isabel","Oscar", "Roberto","Carla")
pacientes2

# Agregamos la variable sexo
sexo<-c("mujer", "hombre", "hombre","mujer",
        "hombre","hombre","mujer")    
sexo
...
Datoslab2<-data.frame(pacientes2,
                      bilirrubina2,
                      sexo)
...
disp_pet<- ggplot(data = Datos_setosa,
...
tabla_iris<-iris %>%   
            group_by(Species) %>%
            summarise(medias = mean (Petal.Length),
                      medianas = median(Petal.Length),
            maximos=max(Petal.Length),
            minimos = min(Petal.Length),   
