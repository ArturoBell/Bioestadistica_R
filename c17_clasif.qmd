...
1. **Dividir los datos en datos de entrenamiento y prueba**. Para esto utilizaremos las funciones `rsample::initial_split()` y `rsample::training()`. Además, remuestrearemos los datos de entrenamiento para realizar la validación cruzada y optimizar los hiper-parámetros del modelo con la función `rsample::vfold_cv()`
...
:::{.callout-note}
Una lista completa de todos los tipos de modelos la puedes encontrar en la [documentación correspondiente](https://www.tidymodels.org/find/parsnip/) de `tidymodels`.
:::
...
11. **Elaborar el reporte** ¿Cómo reportamos todo esto? Te recomiendo que le eches un ojo a @EnriquezGarciaetal_2022 para ver cómo describir el método, pero los resultados podemos resumirlos como:
