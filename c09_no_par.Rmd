---
title: "Técnicas No Paramétricas"
...
**[VIDEO](https://youtu.be/LIVxzEVuY6o)**
...
Estas técnicas reciben esos nombres debido a que no están basadas en los Parámetros de una Población ($\mu$ o $\sigma^2$) y que en consecuencia no requieren que los conozcamos o estimemos.
...
```
```{r, echo = FALSE}
rmarkdown::paged_table(head(bloques))
```
...
```
```{r, echo = FALSE}
rmarkdown::paged_table(head(abundancias))
```
...
2. La variable independiente será una variable categórica con dos grupos
...
5. Aunque los datos no se ajusten a una distribución normal, deberán de tener la misma forma (ambos con forma de campana y sesgo negativo, o ambos con sesgo positivo, etc.)
