...
Recordemos que utilizaremos un ANOVA factorial de dos vías cuando tengamos una variable dependiente (continua) y dos variables independientes (categorías/grupos). A diferencia de realizar ANOVAs de una vía independientes (infla $\alpha$), cuyas inferencias estarán en términos únicamente de las diferencias o no entre la variable con respecto a los grupos, también podemos analizar la interacción entre las variables. La prueba Sheirer-Ray-Hare es una extensión a dos categorías del ANOVA Kruskal-Wallis; es decir, la alternativa no paramétrica al ANOVA de dos vías. Al igual que en Kruskal-Wallis, la prueba post-hoc recomendada es la prueba de Dunn para cada factor en el que se concluya que hay diferencias significativas. 

Ahora implementémosla en R. El primer paso es tener datos adecuados, para lo cual utilizaremos los datos `milipedos.csv`. Estos datos se "ajustan" a una distribución normal en todos sus niveles, por lo que será un buen ejemplo para comparar el desempeño de una prueba paramétrica cuando se cumplen los supuestos vs. una prueba no paramétrica.
...
for (i in seq_along(cols)) {# Cicla entre las columnas indicadas
  ...
  if (i == 2) { # Si es la segunda columna
    for (k in seq_along(sexos)) { # Cicla entre los valores de la columna Sexo
...
Si consideramos $\alpha = 0.05$ como valor *CRÍTICO* para nuestras comparaciones únicamente pareciera haber diferencias entre sexos (sin interacción Sexo-Especie), por lo que solo realizaríamos la prueba post-hoc con respecto a esa variable; sin embargo, al tener únicamente dos niveles (machos-hembras), la prueba sería redundante. Para fines del ejercicio supongamos que nuestro nivel de significancia es $\alpha = 0.1$. Bajo este criterio existen diferencias entre especies, pero para saber que par(es) presenta(n) las diferencias habrá que realizar la prueba de Dunn. En este caso, $P.adj = P.unadj * k$, donde k es el número de comparaciones. Lo anterior es equivalente a la corrección que habíamos mencionado antes ya que: $p < \alpha_{adj} = p < \frac{\alpha}{k} \therefore p\times k < \alpha$
...
summary(res.mili)
```
¡SORPRESA! Ahora encontramos diferencias en ambos niveles, aunque no se encontró una interacción entre los factores. Realicemos la prueba HSD de Tukey para analizar las diferencias a cada nivel. Es importante notar que la función `TukeyHSD()` ya proporciona los valores de p ajustados, aunque en este caso no es utilizando la corrección de Bonferroni ni ninguna otra, si no que fue diseñada desde un inicio para mantener bajo control la tasa de error FWER. En esencia es una prueba t de Student $q_s = \frac{Y_A - Y_B}{SE}$ donde $Y_A$ representa la media más grande de la comparación, $Y_B$ la más pequeña y $SE$ el error estándar. La distribución del estadístico de prueba es la distribución del rango Studentizado. En pocas palabras, no necesitamos corregir el valor de $\alpha$.
...
```
```{r}
mvn(iris, subset = "Species", mvnTest = "royston", desc = F)$univariateNormality
```
...
mod.HSD <- mod.HSD$group
mod.HSD
```

### Comparación Multivariada:
La función `hotelling.test()` de la librería `Hotelling` puede recibir una `formula`; sin embargo, el cálculo de los grados de libertad no es el adecuado y, por tanto, todo el cálculo del estadístico de prueba y valor de p. Para evitar ese error, habrá que extraer en dos objetos los datos de cada grupo:
```{r}
...
1. Independencia entre observaciones (instancias)
2. Normalidad multivariada
3. Igualdad de matrices de covarianza para las variables dependientes
...
car::Manova(model, test.statistic = "Wilks")
```
...
Veamos la dispersión multivariada (**Mañana veremos PCA y MDS**)
```{r}
...
Acabamos de ver algunos casos en los que queremos conocer la relación entre una serie de variables dependientes continuas con respecto a uno o más factores categóricos, pero ¿qué pasa si estoy interesado en **predecir** una variable dependiente continua a partir de múltiples variables independientes continua? Habíamos mencionado que ese tipo de problemas son problemas de regresión; es decir, buscaremos una ecuación matemática. En otras palabras, estamos hablando de regresiones multivariadas. Existen diversos algoritmos de aprendizaje automatizado que nos permiten hacer regresiones, desde una regresión lineal múltiple/multivariada hasta la aplicación de aprendizaje profundo con redes neuronales. En esta clase hablaremos del primer caso.

## Regresión Lineal Múltiple

Para esta sección utilizaremos los datos de la base `crime.csv`. Esta base contiene información relacionada con actos violentos (88 variables) y la variable que nos interesa predecir es el # de crímenes/población (columna ViolentCrimesPerPop). 
...
Apliquemos un modelo de regresión lineal múltiple incluyendo todas las variables, sin dividir los datos en entrenamiento-prueba y sin escalarlos. Tampoco filtraremos los datos para evitar una fuga de información (es un ejemplo)
...
#coef(crime.lm)
```
Vemos que el $R^2$ de 0.67 parece "aceptable"; es decir, estamos explicando el 67% de la varianza y predecir el número de crímenes es un problema sumamente complejo que posiblemente no pueda ser definido de manera lineal, veamos qué pasa ahora si aplicamos este mismo modelo realizando la división entrenamiento-prueba.
...
Vemos que no hay demasiadas diferencias con respecto al primer caso. Apliquemos la regresión Lasso:
...
Esta base de datos consiste en distintas mediciones de abulones: Longitud, Diámetro, Altura, Peso Entero, Peso Desconchado, Peso de las vísceras, Peso de la cáscara y la edad del organismo. El objetivo de esta tarea es tratar de predecir la edad de los organismos a partir de las distintas mediciones. Realiza el modelo de RLM de manera en que puedas comprobar si el modelo está sobre o infra ajustado, presenta al menos dos gráficos diagnósticos de la regresión, menciona cuáles son las variables más importantes. ¿Hay variables que sobren? ¿Consideras que hay algún pre-procesamiento de los datos que pueda mejorar el desempeño del modelo? De los métodos que hemos visto a lo largo del curso, ¿Crees que haya alguna técnica de modelación que pudiera tener un mejor desempeño?

```{r}
download <- RCurl::getURL("https://storage.googleapis.com/download.tensorflow.org/data/abalone_train.csv")
abul_data <- read.csv(text = download, header = F)
colnames(abul_data) <- c("Long", "Diam", "Alt", "PesoEnt", "PesoDesc", "PesoVisc", "PesoCasc", "Edad")
head(abul_data)
```
