...
- $H_0: \mu_1 = \mu_2; \bar{X_A} = \bar{X_C}$
- $H_1: \mu_2 ≠ \mu_2; \bar{X_A} ≠ \bar{X_C}$
...
En la [primera]() sesión dimos algunas definiciones de estadística, pero hay una que, toma mucho sentido en el tema que vamos a abordar hoy. Esta fue propuesta por @Savage_1954: "[La estadística] es la ciencia de tomar **decisiones** bajo situaciones de **incertidumbre**.
...
Esto me llevaría al siguiente punto de discusión, sonre el obscurantismo alrededor del valor de p, pero tomemos un desvío para hablar sobre otro concepto muy relacionado con el valor de p: los intervalos de confianza

## Intervalos de confianza

Los intervalos de confianza, como recordarás que mencionamos someramente en la sesión de [estadística descriptiva](), representan una medida de incertidumbre en alguna estimación, pero vayamos más a fondo. Estos intervalos, al igual que el valor de p, abordan el mismo problema: las estimaciones puntuales no son infalibles ni siempre son verídicas, por lo que necesitamos una referencia de "certeza" alrededor de ellas. Si lo piensas detenidamente eso justamente es lo que hacemos con el valor de p y, de hecho, podemos pensar en los intervalos de confianza como el valor de p visto desde otra perspectiva.

Entonces, ¿qué son los intervalos de confianza? Son una referencia de la "certeza" que tenemos alrededor de una estimación. Es muy posible que te hayas encontrado con la notación $\bar{x} ± SD$, en la cual resumimos nuestros datos con el promedio y la desviación estándar de los datos. Esto está bien siempre y cuando solo nos interese describir todos nuestros datos. Si nos interesa dar información sobre nuestra estimación, entonces haríamos algo tal que $\bar{x}; [IC_i, IC_s]$, donde $IC_i$ representa el límite inferior del intervalo de confianza e $IC_s$ el límite superior.
...
Para ejemplificarlo utilicemos 15 muestras de nuestros 10000 datos simulados bajo una distribución normal estándar (`v1$var`):
...
$$
Donde: $\bar{x}$ es el promedio de nuestra muestra, $\mu_0$ es un valor de referencia con el que queremos comparar $\bar{x}$, $s$ es la desviación estándar de la muestra y $n$ es el número de observaciones. Esta ya la aplicamos antes cuando estimamos los intervalos de confianza con la función `t.test`, pero volvamos a visitarla:
...
$$
t = \frac{\bar{x_1} - \bar{x_2}}{sp\cdot\sqrt{\frac{1}{n_1}+\frac{1}{n_2}}}
$$
...
$$
t = \frac{\bar{X}_D - \mu_o}{\frac{S_D}{\sqrt n}}
$$

Donde $\bar{X_D}$ y $S_D$ son el promedio y la desviación estándar de las diferencias. Hagamos el ejercicio. Primero, carguemos los datos, que en este caso están contenidos en un archivo de excel:
