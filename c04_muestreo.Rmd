...
**NOTA**: En la sesión presencial estos conceptos son abordados de manera dinámica, a manera de debate en la cuál se diseña el muestreo de ejemplo desde cero. Hacer esto en un video es imposible, pero trataré de hacer la explicación igual de amena.

**NOTA 2**: El diseño experimental o de muestreo es un tema lo suficientemente grande como para elaborar un curso sobre eso. Únicamente revisaremos los conceptos más importantes, de manera que puedas reflexionar un poco más sobre qué debes tomar en cuenta en tus diseños.
...
3. Precisión: Tratar de tener el mayor número de distinciones posibles para tener una mejor descripción. En nuestras orcas es mejor medir en 830 cm que redondear a 8 m.
...
pobl_dens <- ggplot(data = pobl, aes(x = lt)) +
             ...
             theme_bw() +
             geom_vline(xintercept = media_real, color = "forestgreen") +
...
Aquí lo que hicimos fue tomar un individuo de nuestra población de manera uniformemente "aleatoria", en este caso el número 267 y medirlo. Ahora tenemos que repetir esa acción otras 49 veces, una a una, o podemos decirle a R que lo haga en un solo paso (imaginemos que somos nosotros los ) Podemos hacer esto 50 veces, una a una, o podemos decirle a R que las obtenga todas al mismo tiempo:
...
```{r}
pobl_dens + geom_vline(xintercept = mean(muestras), color = "firebrick")
```
...
```{r}
muestras <- c(sample(machos, size = 35, replace = F), sample(hembras, size = 15, replace = F))
pobl_dens + geom_vline(xintercept = mean(muestras), color = "Firebrick")
```
...
```{r}
pobl_dens + geom_vline(xintercept = mean(c(mean(muestras[1:35]), mean(muestras[36:50]))),
                       color = "Firebrick")
```
...
pobl_dens + geom_vline(xintercept = mean(c(muestras[1:10], muestras[11:15])),
                       color = "Firebrick")
```
